...
const BORDER_STYLES = [
    { label: __('None', 'news-ticker-block'), value: 'none' }, { label: __('Solid', 'news-ticker-block'), value: 'solid' },
    { label: __('Dashed', 'news-ticker-block'), value: 'dashed' }, { label: __('Dotted', 'news-ticker-block'), value: 'dotted' },
];


export default function ContainerStylingPanel({ attributes, setAttributes }) {
    const { tickerStyles } = attributes;
    // Ensure currentTickerStyles is always an object, merging with defaults if necessary
    const defaultStyles = attributes.tickerStyles.default || {}; // Assuming block.json defaults are accessible this way
